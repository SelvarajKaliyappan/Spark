package org.inceptez.retail.analytics
...
object frustrationScoring 
{
    ...
    def getRdbmsData(sqlc: SQLContext,DatabaseName: String, TableName: String, PartitionColumn:String, ConnFile: String):DataFrame = 
    {
      ...
      val Pass   = conn.getProperty("password")
    ...
    } //End of getRdbmsData method
    
    def processCustPayData(sqlc: HiveContext) = 
    {
      //processing customer details and registering as custdetcomplextypes
      sqlc.sql("""select customerNumber,customerName,concat(contactFirstName,' ',contactLastName) as contactfullname,addressLine1,city,state,
        postalCode,country,phone,creditLimit,checknumber,amount,paymentdate from cust_payment """)
      .createOrReplaceTempView("custdetcomplextypes")
      
      //processing order details and registering as orddetcomplextypes
      sqlc.sql("""select customernumber,ordernumber,shippeddate,status,comments,productcode,quantityordered,priceeach,orderlinenumber,productName,
        productLine,productScale,productVendor,productDescription,quantityInStock,buyPrice,MSRP,orderdate from order_products""")
      .createOrReplaceTempView("orddetcomplextypes")
      
      //getting customer payment detail and saving as CSV file
      val custpaymentcomplextypes=sqlc.sql("""select CONCAT(customerNumber,'~',checknumber,'~',CONCAT(creditLimit,'$',amount),'~',paymentdate) 
        from cust_payment""")
      ...
      sqlc.sql("""create external table IF NOT EXISTS retail_mart.custordfinal (customernumber STRING, customername STRING, 
        contactfullname string, addressLine1 string,city string,state string,country string,phone bigint,creditlimit float,
        checknum string,checkamt int,ordernumber STRING,shippeddate date,status string, comments string,productcode string,
        quantityordered int,priceeach double,orderlinenumber int,productName STRING,productLine STRING,productScale STRING,
        productVendor STRING,productDescription STRING,quantityInStock int,buyPrice double,MSRP double,orderdate date) 
        stored as orcfile location 'hdfs://localhost:54310/user/hive/warehouse/retail_mart.db/custordfinal/'""")
      
      //Processing cust and order data and loading into orc table
      val custfinal = sqlc.sql("""insert into retail_mart.custordfinal select cd.customernumber customernumber,cd.customername customername,
        cd.contactfullname contactfullname,cd.addressLine1 addressLine1,cd.city city,cd.state state,cd.country country,
        cast(cd.phone as bigint) phone,cast(cd.creditlimit as float) creditlimit,cd.checknumber checknum,cast(cd.amount as int) checkamt,
        o.ordernumber ordernumber,cast(o.shippeddate as date) shippeddate,o.status,o.comments,o.productcode,
        cast(o.quantityordered as int) quantityordered,cast(o.priceeach as double) priceeach,cast(o.orderlinenumber as int) orderlinenumber,
        o.productName,o.productLine,productScale,o.productVendor,o.productDescription,cast(o.quantityInStock as int) quantityInStock,
        cast(o.buyPrice as double) buyPrice,cast(o.MSRP as double) MSRP,cast(o.orderdate as date) orderdate 
        from custdetcomplextypes cd inner join orddetcomplextypes o on cd.customernumber=o.customernumber""")
      
      val custfinales = sqlc.sql("""select cd.customernumber customernumber,cd.customername customername,cd.city city,
        cast(o.quantityInStock as int) quantityInStock,cast(o.MSRP as double) MSRP,cast(o.orderdate as date) orderdate 
        from custdetcomplextypes cd inner join orddetcomplextypes o on cd.customernumber=o.customernumber""")
    ...
    } //End of processCustPayData method
    ...
    def main( args: Array[String]) {
      ...
      val confile = "/home/hduser/sparkdatalakeprj/retailorders/connection.prob"
      ...
      val custSql = """(select c.customerNumber, upper(c.customerName) as customerName,c.contactFirstName,c.contactLastName,c.phone,
        c.addressLine1,c.city as city,c.state,c.postalCode,c.country,c.salesRepEmployeeNumber,c.creditLimit,p.checknumber,p.paymentdate,
        p.amount from customers c inner join payments p on c.customernumber=p.customernumber) as cust_pay"""
      ...
      val ordSql = """(select o.customernumber,o.ordernumber,o.orderdate as orderdate,o.shippeddate,o.status,o.comments,od.quantityordered,
        od.priceeach,od.orderlinenumber,p.productCode,p.productName,p.productLine,p.productScale,p.productVendor,p.productDescription,
        p.quantityInStock,p.buyPrice,p.MSRP from orders o inner join orderdetails od on o.ordernumber=od.ordernumber 
        inner join products p on od.productCode=p.productCode) as order_prod"""
...
} //End of frustrationScoring Class